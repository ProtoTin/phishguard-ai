...
personal or organization-specific language. Raw data is excluded from Git, the
four deployment artifacts contain fitted features rather than source messages,
and evaluation reports contain aggregate statistics only.

## Artifact handling

Model artifacts are written to `artifacts/models/`. The two reviewed base models
are versioned for deterministic public deployment. Each artifact's SHA-256 digest,
size, Python version, NumPy version, and scikit-learn version are recorded in
`reports/model-evaluation.json`.

Calibrated artifacts are written to `artifacts/policy/`; the two reviewed calibrated
models are also versioned. Their SHA-256 digests and sizes are recorded in the
versioned detection policy and policy-evaluation report.
